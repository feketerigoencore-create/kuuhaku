...
## 真の最終結論（2026-07-03）：独自ドメインでも不可。壁は「送客先の露骨コンテンツ」

cam（Chaturbate）を最後まで詰めた。独自ドメイン `livenight-guide.com` を購入し中身あるLPを載せGEOも設定して再申請（cam-test-03/04）→ **いずれも「Unacceptable offer」で却下（通算7回）。**

担当マネージャー Mike に直接照会して判明：
- Chaturbateは **non-mainstreamなら本来OK**（禁止ではない）。だがチェーンのどこか（LP／転送／最終着地）がフラグ or ルール違反だと却下。
- HilltopAdsは **VirusTotalだけでなく Google Safe Browsing(GSB)** もチェックする。

GSBで確認 → **`livenight-guide.com` も `chaturbate.com` も「安全」**。VTもクリーン。**＝ドメイン起因は完全に否定。**

→ **残った唯一の原因＝Mikeの②「露骨コンテンツ」。** クローラーが最終着地Chaturbate（露骨なcam画面）を見て『禁止コンテンツ』判定。**Chaturbateの中身はアフィリ側に変えられない**ため、**Chaturbate直送客はHilltopAdsで構造的に不可（確定）。**

**$96.8を活かす残りの道（未着手）：** Mike提示の**露骨でないジャンル**（dating / ecommerce / モバイルアプリ / ゲーム / iGaming / sweepstakes / VPN / utilities / pin-submit / OnlyFans）へ切替。**クリーンな独自ドメイン livenight-guide.com を保有済み**なので、「クリーンなオファーリンク」を1つ見つければチェーン全体をクリーンにできる。※dating(MyLead)は転送ドメインが汚れて不可だった点に注意。

**保有資産（再開時）：** HilltopAds残高$96.8／独自ドメイン livenight-guide.com（GSB・VTクリーン・改良版LP設置済み）／Chaturbate・MyLeadアカウント。

---

## ★★★ 突破（2026-07-03）：ついにキャンペーン承認・配信開始 ★★★

**8回目の申請で、初めて審査通過。** `934528 / game-test-01` が **承認済み＋トラフィックオン（配信中）**。

**勝ちの構成（＝これが正解パターン）：**
- オファー：**MyLead「Mini Games - Android - MX」**（CPA $0.26／成果＝インストール＋5ゲーム完了＝軽い／着地＝**Google Play**＝クリーン）
- リンク：`https://sungoclick.space/a/W6GA0fDvw2tm5QL`（VT **1/92**＝無名AV1社のみ・GSB **安全** → Mikeの基準「GSB or 複数AV」をクリア）
- HilltopAds：モバイルpopunder／**Mainstream**／**GEO=Mexico**／**OS=Android**／CPM最小$0.33／1日$20・合計$45

**なぜ通ったか（7回の却下から学んだ正解）：**
1. **非アダルト**オファー → Chaturbateの「露骨コンテンツ」却下を回避
2. **着地がGoogle Play**（大手クリーンドメイン）→ 最終着地のフラグ問題を回避
3. 入口ドメインも**GSB安全＋AV1社のみ** → ドメイン評判クリア
4. 成果が軽い（install＋mini games）→ pop配信で狙える

**次フェーズ：数字を見る（Day3-4）。** HilltopAdsで表示/クリック/消化額、MyLeadでインストール(コンバージョン)を確認。$30前後たまるまで触らない。
